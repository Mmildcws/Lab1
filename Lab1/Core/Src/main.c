...
void ReadMatrixButton_1Row()
{
	...
	for(i=0;i<4;i++)
	{
		Button[(x*4+i)].Current = HAL_GPIO_ReadPin(L[i].PORT,L[i].PIN);
		//Current=Button.Current ;

		if (Button[(x*4+i)].Last ==1 && Button[(x*4+i)].Current==0 ){
		...
		Button[(x*4+i)].Last = Button[(x*4+i)].Current;
		//Last = Button.Last;
